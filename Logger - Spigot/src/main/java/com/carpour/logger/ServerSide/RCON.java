...
public class RCON implements Listener {
    ...
    public void onConnection(RemoteServerCommandEvent event) {
        ...
        if (main.getConfig().getBoolean("Log-Server.RCON")) {
            ...
            //Discord
            if (DiscordFile.get().getBoolean("Discord.Server-Side.RCON.Enable")) {

                Discord.rcon(Objects.requireNonNull(Messages.get().getString("Discord.Server-Side.RCON")).replaceAll("%time%", dateFormat.format(date).replaceAll("%IP%", ip).replaceAll("%command%", command)), false);
            }
