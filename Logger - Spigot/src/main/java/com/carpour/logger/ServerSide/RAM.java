    ...
    public void run() {
        ...
        if (main.getConfig().getInt("RAM.Percent") <= percentUsed) {
            ...
            if (main.getConfig().getBoolean("Log-Server.RAM")) {
                ...
                //Discord
                if (DiscordFile.get().getBoolean("Discord.Server-Side.RAM.Enable")) {

                    Discord.RAM(Objects.requireNonNull(Messages.get().getString("Discord.Server-Side.RAM")).replaceAll("%max%", String.valueOf(maxMemory)).replaceAll("%used%", String.valueOf(usedMemory)).replaceAll("%free%", String.valueOf(freeMemory)), false);
                }
