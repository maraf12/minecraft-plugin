...
public class PortalCreation implements Listener {
    ...
    public void onPortalCreate(PortalCreateEvent event) {
        ...
        if (!event.isCancelled() && main.getConfig().getBoolean("Log-Server.Portal-Creation")) {
            ...
            //Discord
            if (DiscordFile.get().getBoolean("Discord.Server-Side.Portal-Creation.Enable")) {

                Discord.portalCreation(Objects.requireNonNull(Messages.get().getString("Discord.Server-Side.Portal-Creation")).replaceAll("%world%", worldName).replaceAll("%material%", String.valueOf(reason)), false);
            }
